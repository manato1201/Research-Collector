...
| タイミング | 実行内容 | 実行環境 |
|---|---|---|
| 毎日 AM 5:30 | NotebookLM認証更新 | Windowsタスクスケジューラ |
| 毎日 AM 6:00 | 記事収集 → NotebookLM追加 | GitHub Actions |
| 毎週月曜 AM 7:00 | 週次レポート生成（日本語） | GitHub Actions |
| 毎週日曜 AM 5:00 | 認証切れチェック → Issue通知 | GitHub Actions |
...
| 実行 | 最終実行 | 詳細 |
|---|---|---|
| Daily Collect | ✅ 2026-07-03T02:33:51Z | 収集83件 / 新規83件 / NotebookLM追加166件 |
| Weekly Digest | - | - |
<!-- HEALTH_END -->
...
GoogleのCookieは定期的に失効します。このシステムでは以下の方法で対処しています。

```
毎日 AM 5:30（Windowsタスクスケジューラ）
